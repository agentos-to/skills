...
Flow:
  ...
  4. Poll Gmail (agentos@contini.co catch-all) for the magic link email
...
Notes:
  - agentos@contini.co is a catch-all for *@contini.co — Anthropic emails sent to
    agentos@contini.co arrive in agentos@contini.co inbox.
  - Gmail search: use `from:noreply after:TODAY` with account agentos@contini.co
...
def gmail_search_recent_anthropic(account="agentos@contini.co", max_age_minutes=10):
    """
    Search Gmail for recent Anthropic magic link emails.

    Key facts:
    - agentos@contini.co is a catch-all for *@contini.co
    ...
    raise NotImplementedError(
        ...
        'email.search(query="from:anthropic after:TODAY", account="agentos@contini.co") '
...
def do_login_flow(ws_url, email_addr, verbose=False):
    ...
    4. Wait for "Secure link" email in Gmail (agentos@contini.co catch-all)
...
def main():
    ...
    parser.add_argument("--email", default="agentos@contini.co",
                        help="Email address to log in with (default: agentos@contini.co)")
    parser.add_argument("--gmail-account", default="agentos@contini.co",
