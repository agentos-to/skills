...
instructions: |
  ...
  Two orgs are available under agentos@contini.co:
  ...
  2. Fill the email input: selector = input[type=email], value = agentos@contini.co
  ...
  5. Search Gmail for the magic link:
       skill: gmail
       tool: email.search
       params: { query: "from:anthropic after:YYYY/MM/DD", account: "agentos@contini.co" }
       IMPORTANT: agentos@contini.co is a CATCH-ALL for *@contini.co — ALL emails sent
       to agentos@contini.co arrive at agentos@contini.co. Always use agentos@contini.co.
  6. Get the raw email to extract the magic link:
       skill: gmail
       tool: get_raw
       params: { id: MESSAGE_ID, account: "agentos@contini.co" }
  ...
  login:
    description: |
      Full automated login flow for claude.ai. Orchestrates:
        ...
        2. Gmail skill → poll agentos@contini.co (catch-all) for magic link email
      ...
      GMAIL CATCH-ALL NOTE:
        agentos@contini.co → arrives at agentos@contini.co (catch-all for *@contini.co)
        Always search Gmail with account="agentos@contini.co", NOT "agentos@contini.co"
        (agentos@contini.co is not in Mimestream and has no direct Gmail access)

      MAGIC LINK EXTRACTION:
        1. email.search: query="from:anthropic after:YYYY/MM/DD", account="agentos@contini.co"
        2. get_raw: id=MESSAGE_ID, account="agentos@contini.co"
...
|---------|-----|-------------|
| `personal` | agentos@contini.co's Organization | Chat (web history) |
...
- Gmail access via `agentos@contini.co` (catch-all for `*@contini.co`)
